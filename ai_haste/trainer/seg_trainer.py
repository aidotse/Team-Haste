...
class SegTrainer:
    ...
    def _train_epoch(self, dataloader, optimizer, criterions, epoch):
        ...
        for batch_idx, sample in enumerate(dataloader):
            ...
            #print(torch.count_nonzero(output.detach().cpu()))
            losses = []
            loss = 0

            loss_segmentation = criterions[0]["loss"](output_seg, mask.long())
    ...
    def _valid_epoch(self, dataloader, criterions, epoch):
        ...
        with torch.no_grad():
            for batch_idx, sample in enumerate(dataloader):
                ...
                output_seg = torch.argmax(torch.nn.Softmax(dim=1)(output_seg), dim=1).unsqueeze(0)
    ...
    def infer_full_image(self, input, C_out, mask, kernel_size=512, stride=512):
        B, C, W, H = input.shape
        ...
        seg = seg.view(2, n_w, n_h, w, h)
        seg = seg.permute(0, 1, 3, 2, 4).contiguous()
        seg = seg.view(2, output_h, output_w)
