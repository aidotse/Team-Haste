...
class DenseUNetDual(BaseModel):
    ...
    def __init__(
        ...
        self.nb_classses_dual = nb_classses_dual
        ...
        if include_top:
            self.final_conv2 = nn.Conv2d(
                in_channels=channels_count,
                out_channels=nb_classses_dual,
                kernel_size=1,
                stride=1,
                padding=0,
                bias=True,
            )
            self.final_activation2 = self.activation_func2
    ...
    def forward(self, x):
        ...
        skip1 = skip_connections[0]
        x1 = self.transitions_up[0](x, skip1)
        x1 = self.layers_up[0](x1)

        for i in range(1, len(self.up_blocks)):
            skip1 = skip_connections[i]
            x1 = self.transitions_up[i](x1, skip1)
        ...
        x2 = self.transitions_up2[0](x, skip2)
        x2 = self.layers_up2[0](x2)

        for i in range(1, len(self.up_blocks)):
            skip2 = skip_connections.pop()
            x2 = self.transitions_up2[i](x2, skip2)
