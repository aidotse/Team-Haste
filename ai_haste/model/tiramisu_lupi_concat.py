...
class DenseUNetLUPICONCAT(BaseModel):
    """DensUNet
    ...
    def __init__(
        ...
        for i, block_size in enumerate(self.up_blocks[:-1]):
            ...
            channels_count = prev_block_channels
            if i > 0:
                first_decoder_channel_count.append(channels_count)
        ...
        if include_top:
            ...
            nn.Conv2d(
                ...
                out_channels=int(channels_count/4),
                kernel_size=3,
                stride=1,
                padding=1,
                bias=True
            ...
            nn.Conv2d(
                in_channels=int(channels_count/4),
        ...
        for i, block_size in enumerate(self.up_blocks[:-1]):
            ...
            channels_count2 = prev_block_channels2 + (first_decoder_channel_count.pop(i - 1) if i > 0 else 0)
        ...
        channels_count2 = prev_block_channels2 + first_decoder_channel_count.pop()
        self.layers_up2.append(
            DenseBlock(
                in_channels=channels_count2,
                growth_rate=growth_rate,
        ...
        if include_top:
            ...
            nn.Conv2d(
                in_channels=channels_count2,
                out_channels=int(channels_count2/2),
                kernel_size=3,
                stride=1,
                padding=1,
                bias=True
            ...
            nn.Conv2d(
                in_channels=int(channels_count2/2),
                out_channels=int(channels_count2/4),
                kernel_size=3,
                stride=1,
                padding=1,
                bias=True
            ...
            nn.Conv2d(
                in_channels=int(channels_count2/4),
    ...
    def forward(self, x):
        ...
        x1 = self.transitions_up[0](x)
        x1 = self.layers_up[0](x1)

        decoder_sum_connections = []
        for i in range(1, len(self.up_blocks)):
            x1 = self.transitions_up[i](x1)
        ...
        x2 = self.transitions_up2[0](x)
        x2 = self.layers_up2[0](x2)

        for i in range(1, len(self.up_blocks)):
            sum_layers = decoder_sum_connections.pop(0)
            x2 = self.transitions_up2[i](x2)
            x2 = self.layers_up2[i](torch.cat([x2,sum_layers], dim=1))
