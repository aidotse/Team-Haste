...
class DenseUNetLUPILWM(BaseModel):
    """DensUNet
    ...
    def __init__(
        ...
        if include_top:
            self.final_conv2 = nn.Sequential(
                ...
                nn.Conv2d(
                    in_channels=channels_count + 2 + 2,
                    out_channels=nb_classes,
                    kernel_size=1,
                    stride=1,
                    padding=0,
    ...
    def forward(self, x):
        ...
        for i in range(0, len(self.up_blocks)):
            if i == 0:
                decoder_sum_connections.append(x)
                x1 = self.transitions_up[i](x)
            else:
        ...
        for i in range(0, len(self.up_blocks)):
            ...
            if i == 0:
                x2 = self.transitions_up2[i](x + sum_layers)
            else:
                x2 = self.transitions_up2[i](x2 + sum_layers)
