...
class BurstDenseUNetSeg(BaseModel):
    """DensUNet
    ...
    def __init__(
        ...
        prev_block_channels = growth_rate * bottleneck_layers * 2 # if self.nr_global_pooling_layers != len(self.down_blocks) else growth_rate * bottleneck_layers * 2
        ...
        for i, block_size in enumerate(self.up_blocks[:-1]):
            ...
            channels_count = prev_block_channels + skip_connections[i] #* 2
            ...
            prev_block_channels = growth_rate * block_size #* 2
            channels_count += prev_block_channels

        # #prev_block_channels = 0
        # i_prev = i
        # if self.nr_global_pooling_layers != len(self.down_blocks):

        #     for i, block_size in enumerate(self.up_blocks[:-1][nr_global_pooling_layers:]):
        #         i_temp = i_prev + (i + 1)
        ...
        #             )
        #         )
        #         channels_count = prev_block_channels + skip_connections[i_temp]  #* 2
        #         self.layers_up.append(
        #             DenseBlock(
        #                 in_channels=channels_count,
        #                 growth_rate=growth_rate,
        ...
        #         )
        #         prev_block_channels = growth_rate * block_size #* 2
        #         channels_count += prev_block_channels #*2
        ...
        channels_count = prev_block_channels + skip_connections[-1] #* 2
        self.layers_up.append(
            DenseBlock(
                in_channels=channels_count,
                growth_rate=growth_rate,
        ...
        if include_top:
            ...
            self.final_conv_seg = nn.Sequential(
                nn.Conv2d(
                in_channels=channels_count,
                out_channels=int(channels_count/2),
                kernel_size=3,
                stride=1,
                padding=1,
                bias=True
                ...
                nn.Conv2d(
                in_channels=int(channels_count/2),
                out_channels=nb_seg_classes,
                kernel_size=1,
                stride=1,
                padding=0,
    ...
    def concat2(self, x, max_, y):
        ...
        #max_ = max_.repeat(1, im, 1, 1, 1).view(b * im, c1*(1+self.k), h, w)
    ...
    def forward(self, x):
        ...
        order = np.random.choice(x.shape[1], size=x.shape[1], replace=False)
        x = x[:,order,:,:,:]
        ...
        x = x.view((b, n_images, -1, h, w))
        max_x = self.globalmaxpool(x)
        #x = torch.squeeze(max_x, 1)
        #print(x.shape)
        x = self.concat(x, max_x)
        
        # print(x.view((b, n_images, -1, h, w)).shape, max_x.shape, x1.view((b, n_images, -1, h, w)).shape )
        # x = self.concat2(x.view((b, n_images, -1, h, w)), max_x, x1.view((b, n_images, -1, h, w)))
        # print('x: ', x.shape )
        # print(max_x.shape)
        # print(skip_connections.pop().shape)
        #print('Upsamling..')
        #print('Up...')
        for i in range(len(self.up_blocks)):
            #print('non burst lane..')
        ...
        if self.include_top:
            ...
            y_pred_seg = self.final_conv_seg(x)

            if self.final_activation:
                return self.final_activation(y_pred), y_pred_seg
            else:
                return y_pred, y_pred_seg
        else:
            return x
