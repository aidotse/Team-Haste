...
class DenseUNetDualSeg(BaseModel):
    """DensUNet
    ...
    def __init__(
        ...
        if include_top:
            ...
            self.final_conv_reg = nn.Sequential(
            nn.Conv2d(
                in_channels=channels_count,
                out_channels=int(channels_count/2),
                kernel_size=3,
    ...
    def forward(self, x):
        ...
        if self.include_top:
            ...
            y_pred_reg = self.final_conv_reg(x)
            y_pred_seg = self.final_conv_seg(x)

            if self.final_activation:
                return self.final_activation(y_pred_reg), y_pred_seg
            else:
                return y_pred_reg, y_pred_seg
        else:
            return x
