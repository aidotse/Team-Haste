...
class DenseUNetLUPIConcat(BaseModel):
    ...
    def __init__(
        ...
        for i, block_size in enumerate(self.up_blocks[:-1]):
            ...
            channels_count2 = prev_block_channels2 + first_decoder_channel_count[i]
        ...
        channels_count2 = prev_block_channels2 + first_decoder_channel_count[-1]
        self.layers_up2.append(
            DenseBlock(
                in_channels=channels_count2,
                growth_rate=growth_rate,
        ...
        if include_top:
            self.final_conv2 = nn.Sequential(
                ...
                nn.Conv2d(
                    in_channels=channels_count2 + (2 * 2),
    ...
    def forward(self, x):
        ...
        # x1 = self.transitions_up[0](x, skip, use_skip=True)
        ...
        for i in range(0, len(self.up_blocks)):
            ...
            if i == 0:
                x2 = self.transitions_up2[i](x, None, use_skip=False)
            else:
                x2 = self.transitions_up2[i](x2, None, use_skip=False)
